...
本赛道设立攻防赛程，在生成阶段，参赛队伍可以根据给定的文本和参考音频，以高自然度、高相似度为目标生成对应的伪造语音。在本次赛事中我核心方案还是在目前较优的开源模型index-tts上进行人声克隆，加以分析参考音频中各种类型的分别处理适配index-tts希望能达到较好的参考音频处理，如截断、语音增强、多人声、归一化处理等，对于较短音频曾经试过重复缝合，但效果不佳。然后设置不同的参数配置在index-tts中生成多组音频，再进行对比合并得到较优的音频组。
